function [DIC] = ch_dic ( DATA, ROI, THETA_START, THETA_INTERVAL, THETA_END )
            ...
            for k = 1 + j : iter_number
                tot_pair = tot_pair + 1;
                 cosine_sum_value = cosine_sum_value + ...
                        abs(cos((angular(j) - angular(k))* THETA_INTERVAL *pi/180));
            end
        end
        img_mask(row(i),col(i)) = 0;
        img_dic(row(i),col(i),iter) = 1 - cosine_sum_value/tot_pair;
    end
